...
def test_clean_and_validate_hours_basic():
    df = pd.DataFrame(
        {"temp": [20, -70, 100, None, 0], "humidity": [50, 30, None, 0, 0]},
        index=[
            "2025-01-01 00:00",
            "2025-01-01 01:00",
            "2025-01-01 02:00",
    ...
    assert (
        not (
            (result.drop(columns=["time"]) == 0)
            | (result.drop(columns=["time"]).isna())
        )
        .all(axis=1)
        .any()
    )
...
def test_clean_and_validate_hours_coco_column():
    df = pd.DataFrame(
        {"temp": [25], "coco": ["1", "2", None, "a", "3.0"][:1]}, index=[0]
    )
    result = clean_and_validate_hours(df)
...
def test_clean_and_validate_days_basic():
    df = pd.DataFrame(
        {"tavg": [20, -70, 100, None, 0], "precip": [0, 1, None, 0, 0]},
