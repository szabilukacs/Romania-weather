...
def test_prepare_to_records():
    ...
    records, df_out = prepare_to_records(df, station_id=5, cols=["col1","col2","station_id"])
...
def test_insert_into_db_mock():
    ...
    with patch("src.utils.utils.execute_values") as mock_execute_values:
        insert_into_db(df, mock_conn, station_id=3, insert_sql=insert_sql, cols=["col1","station_id"])
...
def test_load_data_into_df_mock():
    ...
    with patch("src.utils.utils.connect_to_db") as mock_conn_func, \
